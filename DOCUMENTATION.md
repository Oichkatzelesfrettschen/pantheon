...
Hypothesis (Riemann Resonance)
        ↓
    Prediction (oscillating dark energy at γ₁ = 14.134725)
        ↓
    Data (Pantheon+ 1,701 SNe Ia + DESI 2024 BAO)
        ↓
    Falsification (Δχ² = -24.1)
        ↓
...
|--------|---------|--------------|
| Cosmology | **FALSIFIED** | Riemann resonance ruled out at >3σ |
...
Modern cosmology faces a fundamental discrepancy: the Hubble constant H₀ measured from early-universe probes (CMB, BAO) differs from late-universe measurements (SNe Ia, Cepheids) by ~5σ.
...
Type Ia supernovae arise from thermonuclear explosions of carbon-oxygen white dwarfs near the Chandrasekhar mass limit (M_Ch ≈ 1.44 M☉). Their standardizable luminosity makes them ideal cosmological distance indicators.
...
```
M_B = -19.3 + 0.78 × (Δm₁₅ - 1.1)
```
enables standardization but lacks first-principles explanation.

### 2.3 The Riemann Hypothesis Connection

The Riemann zeta function ζ(s) has non-trivial zeros at s = ½ + iγₙ. The first zero occurs at:

```
γ₁ = 14.134725141734693790...
```

The speculative "Dissociative Field Theory" proposed that vacuum structure is governed by 64-dimensional Chingon algebra, with dark energy oscillating at frequency γ₁. This would produce log-periodic modulation in the dark energy equation of state:

```
w(z) = -1 + A·cos(γ₁·ln(1+z) + φ)
```
...
**Testable Consequence:** BAO measurements at different redshifts should trace a "Riemann snake" pattern in the w₀-wₐ plane.
...
The Spandrel model modifies the standard Friedmann equation by introducing a "stiffness" parameter ε:
...
**Observable Signature:** BAO measurements at z = 0.3, 0.5, 0.7, 1.0 should trace a characteristic "snake" pattern when plotted in the w₀-wₐ plane.
...
**Interpretation:** The Spandrel ε parameter is **degenerate** with Ωₘ. Adding ε provides no statistical improvement.
...
**Interpretation:** The Riemann model is **strongly disfavored**. The oscillatory pattern predicted by γ₁ modulation does not appear in the BAO data.
...
U = [ρ, ρu, E, ρY]ᵀ           (conserved variables)
F = [ρu, ρu² + P, (E+P)u, ρuY]ᵀ  (fluxes)
S = [0, 0, Q·ω, -ω]ᵀ           (source terms)
```
...
```
P = P_deg(ρ) + P_ion(ρ,T) + P_rad(T)
```

**Degenerate Electron Pressure (Chandrasekhar):**
```
P_deg = (π m_e⁴ c⁵)/(3h³) × f(x)

x = p_F/(m_e c)  (relativity parameter)
p_F = ħ(3π²n_e)^(1/3)  (Fermi momentum)

f(x) = x(2x²-3)√(1+x²) + 3·sinh⁻¹(x)
```

**Limiting Cases:**
- x << 1 (non-relativistic): P ∝ ρ^(5/3)
- x >> 1 (ultra-relativistic): P ∝ ρ^(4/3)

**Ion Pressure:**
```
P_ion = (ρ k_B T)/(A_bar m_p)
```
...
```
λ = S(E₀)/E₀ × exp(-3E₀/kT) × √(8/(πμ(kT)³))

E₀ = (b kT/2)^(2/3)  (Gamow peak)
b = 2πη  (Sommerfeld parameter)
```

**Screening Enhancement:**
```
f_screen = exp(ζ)
ζ = (Z₁ Z₂ e²)/(a_12 k_B T) × [plasma corrections]
```
...
Turbulent energy cascades from integral scale L to Kolmogorov scale λ_k:
...
|-------|-------------|--------|
| Linear Spandrel (ε) | ~0 | Inconclusive (degenerate) |
| Riemann Resonance (γ₁) | -24.1 | **RULED OUT** |

**Conclusion:** Neither Spandrel nor Riemann modifications improve upon ΛCDM. The Riemann oscillation is definitively excluded.
...
| Peak temperature | 5×10⁹ K | 5–10×10⁹ K | ✓ |
| Ni-56 mass | 1.04 M☉ | 0.4–0.9 M☉ | ~ |
| M_B (peak) | -19.9 | -19.3 ± 0.5 | ✓ |
...
**Chandrasekhar EOS:**
```
P_deg = (πm_e⁴c⁵)/(3h³) × [x(2x²-3)√(1+x²) + 3sinh⁻¹(x)]
```
...
```
u_sp = |∇T|⁻¹ × |∂T/∂t|_burn ≈ c_s → DDT
```

**Arnett's Rule:**
```
L_peak ≈ ε_Ni × M_Ni / τ_Ni
```
