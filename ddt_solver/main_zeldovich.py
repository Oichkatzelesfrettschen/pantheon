...
from constants import K_BOLTZMANN, M_PROTON, Q_BURN
...
class SimulationConfig:
    ...
    # Initial conditions
    rho_ambient: float = 2e7       # g/cm³ (DDT-favorable density)
...
class ZeldovichDDTSolver:
    ...
    def _setup_initial_conditions(self):
        ...
        if cfg.verbose:
            ...
            print(f"  ρ = {cfg.rho_ambient:.2e} g/cm³")
    ...
    def _hydro_step(self, dt: float):
        ...
        dU/dt = -∂F/∂x (no source terms in hydro step)
        """
    ...
    def _detect_detonation(self) -> bool:
        ...
        # Detonation criterion: Mach > 1 and significant velocity
        if mach > 1.0 and self.shock_velocity > 1e7:
