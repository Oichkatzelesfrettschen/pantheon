...
conditions (ρ ~ 10^7 g/cm³, T ~ 10^9 K).
...
class EOSState:
    ...
    rho: np.ndarray      # Density (g/cm³)
    T: np.ndarray        # Temperature (K)
    P: np.ndarray        # Total pressure (erg/cm³ = dyne/cm²)
...
def fermi_momentum(n_e: np.ndarray) -> np.ndarray:
    """Fermi momentum p_F = ħ(3π²n_e)^(1/3)."""
...
def pressure_degenerate(rho: np.ndarray) -> np.ndarray:
    ...
    P_deg = (π m_e⁴ c⁵)/(3 h³) * f(x)

    where f(x) = x(2x²-3)√(x²+1) + 3 sinh⁻¹(x)
    ...
    # f(x) = x(2x²-3)√(x²+1) + 3 arcsinh(x)
    sqrt_term = np.sqrt(x**2 + 1.0)
    f_x = x * (2.0 * x**2 - 3.0) * sqrt_term + 3.0 * np.arcsinh(x)

    # Prefactor: (π m_e⁴ c⁵) / (3 h³) = (m_e c²)/(8π²) * (m_e c / ħ)³
    # Using: (m_e c / ħ) = 1/(Compton wavelength / 2π)
...
def pressure_ions(rho: np.ndarray, T: np.ndarray) -> np.ndarray:
    ...
    P_ion = n_ion * k_B * T = (ρ / A_bar m_p) * k_B * T
    """
...
def energy_degenerate(rho: np.ndarray) -> np.ndarray:
    ...
    # Using exact integral: e = (m_e c² n_e / ρ) * g(x)
    # where g(x) involves hypergeometric-like terms

    # Simplified: use thermodynamic relation e = ∫P/ρ² dρ
...
def effective_gamma(rho: np.ndarray, T: np.ndarray) -> np.ndarray:
    """
    Effective adiabatic index γ_eff.

    Interpolates between:
        γ = 5/3 (non-relativistic degenerate)
        γ = 4/3 (ultra-relativistic degenerate)
        γ = 5/3 (ideal ion gas contribution)
    """
    n_e = electron_density(rho)
    p_F = fermi_momentum(n_e)
    x = relativity_parameter(p_F)
    ...
    # γ_deg interpolates from 5/3 to 4/3
...
def eos_from_rho_T(rho: np.ndarray, T: np.ndarray) -> EOSState:
    """
    Complete EOS: given (ρ, T), compute all thermodynamic quantities.
...
def temperature_from_rho_e(rho: np.ndarray, e_int: np.ndarray,
    ...
    """
    Invert EOS: given (ρ, e), solve for T using Newton-Raphson.

    This is the "backward" EOS call needed during hydro evolution
    when we know conserved variables (ρ, ρe) but need T for reactions.
    """
    rho = np.atleast_1d(rho).astype(np.float64)
    ...
    for _ in range(max_iter):
        ...
        # Derivative: de/dT = (3/2) k/(A m_p) + 4 a T³/ρ
...
def eos_from_rho_e(rho: np.ndarray, e_int: np.ndarray,
    ...
    """
    Complete EOS from conserved variables: given (ρ, e), compute all quantities.
...
if __name__ == "__main__":
    ...
    rho_test = np.array([1e6, 1e7, 3e7, 1e8])  # g/cm³
    ...
    for rho, T in zip(rho_test, T_test):
        ...
        print(f"\nρ = {rho:.1e} g/cm³, T = {T:.1e} K")
        print(f"  Relativity param x = {x:.3f}")
        print(f"  P_total = {state.P[0]:.3e} dyne/cm²")
        ...
        print(f"  γ_eff = {state.gamma_eff[0]:.3f}")
