...
from constants import M_SUN, DAY, TAU_NI56, TAU_CO56, Q_BURN
from ddt_solver.eos_white_dwarf import eos_from_rho_T
from ddt_solver.flux_hllc import primitive_to_conserved, conserved_to_primitive
from ddt_solver.reaction_carbon import chapman_jouguet_velocity
...
class NickelYieldAnalyzer:
    ...
    def compute_yields(self, rho: np.ndarray, T_max: np.ndarray = None) -> NucleosynthesisResult:
        ...
        # Mass per cell
        dm = rho * self.dx  # g/cm² (1D) - need to scale for 3D
        ...
        # where ε_Ni = 3.9×10¹⁰ erg/g/s and τ_Ni = 8.8 days
        # Simplified: L_peak ≈ 2×10⁴³ * (M_Ni / M_sun) erg/s
...
def run_yield_analysis():
    ...
    # Get total mass in simulation (g/cm²) and scale to 3D estimate
    rho, _, _ = conserved_to_primitive(solver.U, solver.gamma_eff)
    M_1D = np.sum(rho * solver.dx)  # g/cm² (column mass)
    ...
    print(f"\n1D Column Masses (g/cm²):")
    print(f"  NSE (→ Ni-56):     {result.M_NSE:.3e} g/cm²")
    print(f"  Si-burning:        {result.M_Si_burn:.3e} g/cm²")
    print(f"  O-burning:         {result.M_O_burn:.3e} g/cm²")
    print(f"  C-burning:         {result.M_C_burn:.3e} g/cm²")
    print(f"  Unburned:          {result.M_unburned:.3e} g/cm²")
    print(f"  Total:             {result.M_NSE + result.M_Si_burn + result.M_O_burn + result.M_C_burn + result.M_unburned:.3e} g/cm²")
    ...
    print(f"  Expected for normal SN Ia: ~1-2 × 10⁴³ erg/s")
