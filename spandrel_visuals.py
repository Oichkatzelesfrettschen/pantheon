...
def create_cosmological_veto():
    ...
    # Plot
    ax1.axhline(-1, color='#8b949e', linestyle=':', linewidth=1, label='ΛCDM (w = -1)')
    ax1.fill_between(z, w_DESI_lower, w_DESI_upper, color='#238636', alpha=0.3, label='DESI 2024 (1σ)')
    ax1.plot(z, w_DESI, color='#3fb950', linewidth=2, label='DESI Best Fit')
    ax1.plot(z, w_Riemann, color='#f85149', linewidth=2, linestyle='--', alpha=0.7, label='Riemann γ₁ (FALSIFIED)')
    ...
    ax2.axhline(9.21, color='#ffa657', linestyle='--', linewidth=2, label='3σ rejection threshold')

    # Final verdict box
    ax2.annotate('Δχ² = -24.1\nRIEMANN MODEL\nRULED OUT',
    ...
    ax2.set_ylabel('χ² Penalty (BAO)', fontsize=12)
    ax2.set_ylim(0, 30)
...
def create_zeldovich_triumph():
    ...
    l1, = ax3.plot(x_km, rho / 1e7, color='#8b949e', linewidth=2, label='Density ρ')
    l2, = ax3_twin.plot(x_km, X_C12, color='#3fb950', linewidth=2, label='X(C12)')

    ax3.set_xlabel('Position (km)', fontsize=12)
    ax3.set_ylabel('Density (10⁷ g/cm³)', fontsize=11, color='#8b949e')
