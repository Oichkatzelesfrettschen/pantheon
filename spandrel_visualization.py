...
class CornerPlot:
    ...
    def _plot_2d_contour(self, ax: plt.Axes, x_idx: int, y_idx: int):
        ...
        levels = []
        for sigma_frac in [0.393, 0.865, 0.989]:  # 1σ, 2σ, 3σ in 2D
...
class HubbleDiagramPlot:
    ...
    def plot_comparison(self, models: Dict[str, Dict[str, Any]],
        ...
        for name, model in models.items():
            ...
            label = f"{name}: H₀={params.get('H0', '?'):.1f}, Ωₘ={params.get('Omega_m', '?'):.3f}"
            if 'epsilon' in params and params['epsilon'] != 0:
                label += f", ε={params['epsilon']:.4f}"
...
class StiffnessEvolutionPlot:
    ...
    def plot(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax1.plot(z_range, correction, 'r-', linewidth=2, label=f'ε = {self.epsilon:.4f}')
        ...
        ax1.set_ylabel('Distance Modulus Correction Δμ (mag)')
...
class HubbleTensionPlot:
    ...
    def plot(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax.annotate(f'Tension: {tension:.1f}σ', xy=(70.2, 1.5),
    ...
    def plot_evidence_comparison(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        if len(models) >= 2:
            ...
            if log_bf > 5:
                ...
                interpretation = "Weak for ΛCDM"
            elif log_bf > -2.5:
                interpretation = "Substantial for ΛCDM"
            else:
                interpretation = "Strong for ΛCDM"
...
class Chi2ContourPlot:
    ...
    def compute_chi2_grid(self, H0_range: np.ndarray, param2_range: np.ndarray,
        ...
        from spandrel_cosmology_hpc import VectorizedCosmology, CosmologyParams
    ...
    def plot(self, best_fit: Dict, save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax1.clabel(contour1, fmt={levels[0]: '1σ', levels[1]: '2σ', levels[2]: '3σ'})
        ...
        ax1.axhline(y=0, color='gray', linestyle='--', linewidth=1.5, label='ΛCDM (ε=0)')

        ax1.set_xlabel('H₀ (km/s/Mpc)')
        ax1.set_ylabel('Stiffness ε')
        ax1.set_title('χ² Contours: H₀ vs Stiffness')
        ...
        contour2 = ax2.contour(H0_range, Om_range, chi2_grid2, levels=levels2,
                               colors=['green', 'blue', 'red'])
        ax2.clabel(contour2, fmt={levels2[0]: '1σ', levels2[1]: '2σ', levels2[2]: '3σ'})
        ...
        ax2.set_ylabel('Ωₘ')
        ax2.set_title('χ² Contours: H₀ vs Ωₘ')
