...
class PhillipsObservations:
    ...
    Phillips (1993): Δm₁₅(B) vs M_B(peak)
    """
...
class PhillipsFromTurbulence:
    ...
    The key relations:
        ...
        3. Broader light curve → smaller Δm₁₅
        4. More Ni-56 → brighter peak → more negative M_B

    Therefore: Higher D → smaller Δm₁₅ AND more negative M_B
    ...
    def delta_m15_from_M_Ni(self, M_Ni: float) -> float:
        ...
        Since κ ∝ M_Ni (opacity from Ni/Co), we get:
            Δm₁₅ ∝ 1/M_Ni^α
        """
    ...
    def fit_to_observations(self) -> Dict:
        ...
        # Interpolate model at observed Δm₁₅ values
...
def plot_phillips_derivation(save_path: str = None):
    ...
    ax1.axhline(0.6, color='gray', linestyle='--', alpha=0.5, label='Reference (0.6 M☉)')
    ax1.axvline(2.35, color='gray', linestyle=':', alpha=0.5, label='D = 2.35')
    ax1.set_xlabel('Fractal Dimension D')
    ax1.set_ylabel('Ni-56 Mass (M☉)')
    ...
    ax2.set_xlabel('Ni-56 Mass (M☉)')
    ax2.set_ylabel('Δm₁₅(B) (mag)')
    ...
    ax5.plot(model['delta_m15'], model['M_B'], 'lime', linewidth=2,
            label='Mean relation')
    ax5.set_xlabel('Δm₁₅(B) (mag)')
    ax5.set_ylabel('M_B (peak)')
    ax5.set_title(f'Population (N={pop["n_sne"]}, σ_D={population.D_std})')
    ax5.invert_yaxis()
    ...
    ║    M_Ni → Opacity → τ_diff → Δm₁₅                   ║
    ║    M_Ni → L_peak → M_B (Arnett)                     ║
    ║                                                      ║
    ║  Model Parameters:                                   ║
    ║    D range: [{phillips.D_min:.2f}, {phillips.D_max:.2f}]                         ║
    ║    D reference: {phillips.D_ref:.2f}                            ║
    ║    α (opacity scaling): {phillips.alpha_opacity:.2f}                     ║
    ║                                                      ║
    ║  Fit Quality:                                        ║
    ║    χ² = {fit['chi2']:.1f} ({fit['n_dof']} dof)                            ║
    ║    RMS residual = {fit['rms']:.3f} mag                      ║
    ║                                                      ║
    ║  Population Synthesis:                               ║
    ║    D_mean = {population.D_mean:.2f}, σ_D = {population.D_std:.2f}                   ║
...
if __name__ == "__main__":
    ...
    fit = phillips.fit_to_observations()
    print(f"\nFit to observations:")
    print(f"  χ² = {fit['chi2']:.1f} ({fit['n_dof']} dof)")
    ...
    print(f"  Mean M_Ni: {np.mean(pop['M_Ni']):.3f} ± {np.std(pop['M_Ni']):.3f} M☉")
