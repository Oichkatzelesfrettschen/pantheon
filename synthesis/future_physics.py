...
This module addresses the open questions from the Spandrel Synthesis:
    1. Why is σ_D so narrow? (The "Standard Bomb" problem)
...
The Phillips scatter of ~0.15 mag implies σ_D ≈ 0.03.
...
class WhiteDwarfStructure:
    ...
    The remarkable uniformity of Type Ia progenitors stems from:
        1. Chandrasekhar mass limit (1.44 M☉)
    ...
    def __post_init__(self):
        ...
        # v_conv ≈ (L / ρ × c_p × T)^(1/3) × (g × α × ΔT / T)^(1/2)
        ...
        nu = 1e3  # Degenerate viscosity (cm²/s)
    ...
    def _compute_convective_velocity(self) -> float:
        ...
        # ε_nuc ≈ 10^15 erg/g/s at T = 6×10⁸ K, ρ = 2×10⁹
        epsilon_nuc = 1e15

        # Convective luminosity: L = 4πr² × ρ × v × c_p × ΔT
        # Solving for v:
...
def analyze_standard_bomb_tolerance() -> Dict:
    """
    Investigate why σ_D is so small.
    ...
    Variations come only from:
        1. Mass (within ~0.1 M☉ of M_Ch)
        2. Metallicity (affects Y_e slightly)
        3. Rotation (minor effect on structure)

    These produce only small variations in turbulence → small σ_D.
    """
    print("=" * 70)
    print("THE STANDARD BOMB TOLERANCE")
    print("Why is σ_D ≈ 0.03?")
    print("=" * 70)
    ...
    print(f"\nMass range: {1.35:.2f} - {1.44:.2f} M☉")
    print(f"Central density range: {results[0]['rho_c']:.2e} - {results[-1]['rho_c']:.2e} g/cm³")
    ...
    print(f"\nTHIS IS WHY σ_D IS SMALL:")
    print(f"  → Chandrasekhar mass is a FIXED POINT of stellar evolution")
    print(f"  → All progenitors converge to M ≈ 1.4 M☉ before ignition")
    print(f"  → Central conditions (ρ, T, v_conv) have minimal spread")
...
class GravitationalWaveSignature:
    ...
    which sources gravitational waves. The amplitude depends on:
        - Quadrupole moment: Q_ij ~ M × R² × asymmetry
        - Frequency: f ~ v_turb / L ~ 1-100 Hz
        - Duration: τ ~ R / v_det ~ 1 second
    ...
    def lisa_detectability(self) -> Dict:
        ...
        LISA sensitivity: h ~ 10⁻²¹ at f ~ 0.01 Hz
        ...
        # Stochastic background: Ω_GW ~ (f × h²) × rate × τ
...
def analyze_gw_signature() -> Dict:
    ...
    print(f"Asymmetry range: ε ∈ [{results[0]['asymmetry']:.3f}, {results[-1]['asymmetry']:.3f}]")
    print(f"GW strain range: h ∈ [{min(h_range):.2e}, {max(h_range):.2e}]")
...
class LESSpecification:
    ...
    # Grid specification
    n_cells: int = 512           # Per dimension (512³ = 134M cells)
...
def create_les_specification() -> Dict:
    ...
    print(f"\nGrid: {spec.n_cells}³ = {cost['n_cells']:.2e} cells")
    print(f"Domain: {spec.domain_size/1e8:.0f} × 10⁸ cm (full star)")
    print(f"Timesteps: {cost['n_steps']:.2e} (dt = {cost['dt']:.2e} s)")

    print(f"\nPhysics:")
    print(f"  Nuclear network: {'Yes (α-chain)' if spec.include_nuclear else 'No'}")
...
def run_future_physics():
    ...
    print("╠" + "═" * 68 + "╣")
    print("║                                                                    ║")
    print("║  Q1: Why is σ_D ≈ 0.03?                                            ║")
    print("║  A1: Chandrasekhar mass is an ATTRACTOR. All WDs converge to       ║")
    print("║      M ≈ 1.4 M☉ with nearly identical central conditions.          ║")
    print("║                                                                    ║")
    print("║  Q2: Can LISA detect fractal signatures?                           ║")
    print("║  A2: Individual events: NO (h ~ 10⁻²⁴ << 10⁻²¹)                    ║")
    ...
    print("║      Requires 512³ LES simulation (~40 GPU-days on A100)           ║")
    print("║                                                                    ║")
    print("║  PREDICTION: The observed σ_D = 0.03 reflects the uniformity of    ║")
