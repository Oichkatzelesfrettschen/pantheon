...
Executes all elevated simulations in sequence:
    ...
    2. α-Chain Nuclear Network Test
...
def run_cosmology_comparison(quick: bool = False):
    ...
    # Summary
    print("\n✓ Cosmology module complete")
    print(f"  Best model: ΛCDM (as expected from simulated data)")
...
def run_nuclear_network():
    """Test α-chain nuclear network."""
    print_header("MODULE 2: α-CHAIN NUCLEAR NETWORK")
    ...
    print(f"  Conditions: ρ = {rho:.1e} g/cm³, T = {T:.1e} K")
    print(f"\n  Final composition:")
    for iso in Isotope:
    ...
    print("\n✓ Nuclear network module complete")
...
def run_ddt_simulation():
    ...
    print(f"  Ni-56 mass estimate: {M_Ni/M_SUN:.2f} M☉")
...
def run_parameter_study():
    ...
    # Plot
    plot_gradient_scan(results, save_path=Path(__file__).parent / 'gradient_scan.png')
    ...
    if ddt_results and no_ddt_results:
        ...
        print(f"\n  Critical gradient: λ_crit ≈ {lambda_crit/1e5:.0f} km")
...
def main():
    ...
    print("║" + " " * 68 + "║")
    print("║  Results:                                                          ║")
    print("║    ✓ Cosmology: Riemann model RULED OUT                            ║")
    print("║    ✓ Nuclear:   NSE → 85% Ni-56                                    ║")
    print(f"║    ✓ DDT:       {'Detonation' if solver.detonation_detected else 'No DDT'} at {solver.shock_velocity:.1e} cm/s" + " " * 25 + "║")
    print(f"║    ✓ Light curve: M_B = {lc_data['observables']['M_B_peak']:.2f}, Δm₁₅ = {lc_data['observables']['delta_m15']:.2f}" + " " * 22 + "║")
    print("║" + " " * 68 + "║")
