...
The Spandrel Hypothesis proposes that spacetime exhibits "stiffness" (ε) that
modifies the standard ΛCDM distance-redshift relationship. This stiffness
...
class SpandrelCosmology:
    ...
    The Spandrel modification to ΛCDM introduces a "stiffness" parameter ε
    that modifies the distance-redshift relationship:

    μ_spandrel(z) = μ_ΛCDM(z) + ε * f(z)
    ...
    - epsilon: Stiffness parameter (ε = 0 recovers standard ΛCDM)
    """
    ...
    def E(self, z: float) -> float:
        ...
        For flat ΛCDM: E(z) = sqrt(Ω_m(1+z)³ + Ω_Λ)
        """
        return np.sqrt(
    ...
    def distance_modulus_lcdm(self, z: float) -> float:
        """
        Standard ΛCDM distance modulus: μ = 5*log10(D_L/10pc)
        """
    ...
    def spandrel_correction(self, z: float) -> float:
        ...
        - The (1 - 1/(1+z)²) term represents relaxation from early stiffness
        """
        return self.epsilon * np.log(1 + z) * (1 - 1/(1 + z)**2)

    def distance_modulus_spandrel(self, z: float) -> float:
        """
        Spandrel-modified distance modulus: μ_spandrel = μ_ΛCDM + correction
        """
...
class SpandrelFitter:
    ...
    def fit_lcdm(self, initial_guess: Tuple[float, float] = (70.0, 0.3)) -> Dict[str, Any]:
        """
        Fit standard ΛCDM (epsilon = 0).

        Returns dict with H0, Omega_m, chi2, reduced_chi2, p_value
        """
        print("\n" + "="*60)
        print("Fitting Standard ΛCDM Model (ε = 0)")
        print("="*60)
    ...
    def fit_spandrel(self, initial_guess: Tuple[float, float, float] = (70.0, 0.3, 0.0),
        ...
        """
        print("\n" + "="*60)
        print("Fitting Spandrel Cosmology Model (ε free)")
        print("="*60)
        ...
        print(f"  H₀ = {H0_fit:.2f} km/s/Mpc")
        print(f"  Ωₘ = {Om_fit:.4f}")
        print(f"  ε (stiffness) = {eps_fit:.6f}")
        print(f"  χ² = {chi2_val:.2f} (dof = {dof})")
        print(f"  χ²/dof = {reduced_chi2:.4f}")
        print(f"  p-value = {p_value:.4f}")
    ...
    def compute_parameter_errors(self, best_params: Tuple[float, float, float],
        ...
        1σ errors correspond to Δχ² = 1 for one parameter of interest.
        """
        print("\nEstimating parameter uncertainties (Δχ² = 1)...")
    ...
    def likelihood_ratio_test(self, lcdm_result: Dict, spandrel_result: Dict) -> Dict[str, float]:
        ...
        Tests H0: ε = 0 (ΛCDM) vs H1: ε ≠ 0 (Spandrel)

        The test statistic -2Δln(L) = Δχ² follows χ²(1) under the null.
        """
...
class SpandrelVisualizer:
    ...
    def plot_hubble_diagram(self, lcdm_result: Dict, spandrel_result: Dict,
        ...
        ax1.plot(z_model, mu_lcdm, 'b-', linewidth=2,
                label=f'ΛCDM: H₀={lcdm_result["H0"]:.1f}, Ωₘ={lcdm_result["Omega_m"]:.3f}')
        ax1.plot(z_model, mu_spandrel, 'r--', linewidth=2,
                label=f'Spandrel: H₀={spandrel_result["H0"]:.1f}, Ωₘ={spandrel_result["Omega_m"]:.3f}, ε={spandrel_result["epsilon"]:.4f}')

        ax1.set_xscale('log')
        ax1.set_xlabel('Redshift z', fontsize=12)
        ax1.set_ylabel('Distance Modulus μ (mag)', fontsize=12)
        ax1.set_title('Pantheon+ Hubble Diagram: ΛCDM vs Spandrel Cosmology', fontsize=14)
        ...
        ax2.scatter(self.z_obs, residuals_lcdm, c='blue', s=5, alpha=0.3, label='ΛCDM residuals')
    ...
    def plot_stiffness_effect(self, spandrel_result: Dict, save_path: Optional[str] = None):
        ...
        ax.plot(z_range, corrections, 'r-', linewidth=2, label=f'Spandrel correction (ε = {epsilon:.4f})')
        ...
        ax.set_ylabel('Distance Modulus Correction Δμ (mag)', fontsize=12)
    ...
    def plot_chi2_contours(self, fitter: SpandrelFitter, best_params: Tuple[float, float, float],
        ...
        # Contour levels for 1σ, 2σ, 3σ (Δχ² = 2.30, 6.17, 11.8 for 2 params)
        levels = chi2_min + np.array([2.30, 6.17, 11.8])

        contour = ax.contour(H0_range, eps_range, chi2_grid, levels=levels,
                            colors=['green', 'blue', 'red'])
        ax.clabel(contour, fmt={levels[0]: '1σ', levels[1]: '2σ', levels[2]: '3σ'})

        ax.plot(H0_best, eps_best, 'k*', markersize=15, label='Best fit')
        ax.axhline(y=0, color='gray', linestyle='--', linewidth=1, label='ΛCDM (ε=0)')

        ax.set_xlabel('H₀ (km/s/Mpc)', fontsize=12)
        ax.set_ylabel('Stiffness ε', fontsize=12)
        ax.set_title('χ² Confidence Contours: H₀ vs Stiffness', fontsize=14)
        ax.legend(loc='best', fontsize=10)
        ax.grid(True, alpha=0.3)
...
def run_full_analysis(data_path: str = "Pantheon+SH0ES.dat"):
    ...
    print(f"  Δχ² (ΛCDM - Spandrel) = {lr_test['delta_chi2']:.4f}")
    print(f"  p-value = {lr_test['p_value']:.6f}")
    print(f"  Significance = {lr_test['sigma']:.2f}σ")

    if lr_test['prefers_spandrel']:
        print("\n  *** Spandrel model PREFERRED over ΛCDM at 95% confidence ***")
    else:
        print("\n  ΛCDM is statistically sufficient (no evidence for stiffness)")
    ...
    if abs(eps) < 2 * eps_err:
        ...
        print("  Standard ΛCDM is correct. The 'Surface Tension' is effectively zero.")
    ...
    ΛCDM Best Fit:
      H₀ = {lcdm_result['H0']:.2f} km/s/Mpc
      Ωₘ = {lcdm_result['Omega_m']:.4f}
      χ²/dof = {lcdm_result['reduced_chi2']:.4f}

    Spandrel Best Fit:
      H₀ = {spandrel_result['H0']:.2f} ± {errors.get('H0', 0):.2f} km/s/Mpc
      Ωₘ = {spandrel_result['Omega_m']:.4f} ± {errors.get('Omega_m', 0):.4f}
      ε  = {spandrel_result['epsilon']:.6f} ± {errors.get('epsilon', 0):.6f}
      χ²/dof = {spandrel_result['reduced_chi2']:.4f}

    Model Comparison:
      Δχ² = {lr_test['delta_chi2']:.4f}
      Significance: {lr_test['sigma']:.2f}σ
