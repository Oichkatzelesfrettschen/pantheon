...
1. DESI 2024 BAO results (w_0-wₐ constraints)
...
The Ultimate Question:
    Does the "Riemann Snake" w(z) = -1 - (Agamma/3)sin(gammaln(1+z)+phi)
...
RIEMANN_FIT = {
    'amplitude': 0.026237,
    'phase': 2.8332,  # radians (162.3 deg)
    'H0': 73.679,
    'Omega_m': 0.31627
}
...
class RiemannEoS:
    ...
    w(z) = -1 - (Agamma/3) * sin(gamma*ln(1+z) + phi)

    In CPL parameterization (w = w_0 + wₐ*(1-a) = w_0 + wₐ*z/(1+z)):
    We can compute effective w_0 and wₐ by Taylor expansion at z=0.
    """
    ...
    def effective_w0_wa(self) -> Tuple[float, float]:
        ...
        # dw/dz|_{z=0} = -(Agamma^2/3) * cos(phi)
...
def get_quasar_hubble_data():
    ...
    Format: (z, mu, sigma_mu) - redshift, distance modulus, error
...
def get_cosmic_chronometer_data():
    ...
    Format: (z, H(z), sigma_H) in km/s/Mpc
    """
    cc_data = np.array([
        # z,     H(z),   sigma_H
        [0.07,   69.0,   19.6],
...
def get_bao_dv_data():
    ...
    bao_data = np.array([
        ...
        [2.33,   37.6,    1.2],    # eBOSS Lyalpha
        [2.40,   37.3,    1.7],    # DESI Lyalpha (2024)
    ])
...
class ExtendedRiemannCosmology:
    ...
    def comoving_distance(self, z: np.ndarray, n_steps: int = 1000) -> np.ndarray:
        ...
        for i, zi in enumerate(z):
            ...
            result[i] = np.trapezoid(integrand, z_int)
...
def plot_w0_wa_plane(riemann_eos: RiemannEoS, save_path: Optional[str] = None):
    """
    Plot the w_0-wₐ plane with survey constraints and Riemann prediction.

    The "Riemann Snake" shows how w_0 and wₐ vary as phase changes.
    """
    ...
    # Plot the "Riemann Snake" - how (w_0, wₐ) varies with phase
    ...
    ax.plot(w0_fit, wa_fit, 'r*', markersize=25, markeredgecolor='black',
           label=f'Pantheon+ Riemann Fit (phi={np.degrees(riemann_eos.phi):.0f} deg)')

    # Mark LambdaCDM
    ax.plot(-1, 0, 'g^', markersize=15, markeredgecolor='black',
           label='LambdaCDM (w=-1, wₐ=0)')
    ...
    # Add text annotation
    ax.text(-1.7, -2.7,
           f'Riemann: A={riemann_eos.A:.3f}, gamma={riemann_eos.gamma:.2f}\n'
           f'w_0={w0_fit:.3f}, wₐ={wa_fit:.3f}',
...
def plot_w_z_evolution(riemann_eos: RiemannEoS, save_path: Optional[str] = None):
    ...
    ax1.axhline(y=-1, color='blue', linewidth=2, label='LambdaCDM (w = -1)')
    ax1.plot(z, w_riemann, 'r-', linewidth=2.5, label='Riemann Resonance')
    ax1.plot(z, w_cpl, 'g--', linewidth=2, label=f'CPL fit (w_0={w0_fit:.2f}, wₐ={wa_fit:.2f})')

    # DESI best fit
    w0_desi, wa_desi = -0.827, -0.75
    w_desi = w0_desi + wa_desi * z / (1 + z)
    ax1.plot(z, w_desi, 'm:', linewidth=2, label=f'DESI 2024 (w_0={w0_desi}, wₐ={wa_desi})')
    ...
    # Right: Residual from LambdaCDM
    ax2 = axes[1]

    ax2.axhline(y=0, color='blue', linewidth=2, label='LambdaCDM')
    ...
    ax2.set_ylabel('w(z) + 1 (deviation from Lambda)', fontsize=12)
...
def plot_high_z_synthesis(save_path: Optional[str] = None):
    ...
    ax1.plot(z_model, lcdm_cosmo.H(z_model), 'b--', linewidth=2, label='LambdaCDM')
    ...
    ax3.set_ylabel('Distance Modulus mu', fontsize=12)
    ...
    ax4.plot(z_full, delta_mu, 'r-', linewidth=2.5, label='Riemann - LambdaCDM')
    ax4.axhline(y=0, color='blue', linestyle='--', linewidth=1.5, label='LambdaCDM')
    ...
    ax4.set_ylabel('Δmu (mag)', fontsize=12)
...
def compute_chi2_comparison():
    """
    Compute chi-squared for Riemann vs LambdaCDM against all datasets.
    """
    print("\n" + "="*70)
    print("CHI-SQUARED COMPARISON: RIEMANN vs LambdaCDM")
    print("="*70)
    ...
    print(f"\n{'Dataset':<25} {'LambdaCDM chi^2':>12} {'Riemann chi^2':>12} {'Δchi^2':>10} {'N':>6}")
    print("-"*70)
...
def run_desi_riemann_synthesis():
    ...
    Riemann Resonance Parameters (from Pantheon+):
      ...
      Phase phi = {np.degrees(RIEMANN_FIT['phase']):.2f} deg
      Frequency gamma_1 = {GAMMA_1:.4f} (First Riemann Zero)

    Effective CPL Parameters:
      w_0 (analytic) = {w0_analytic:.4f}
      wₐ (analytic) = {wa_analytic:.4f}
      w_0 (fitted) = {w0_fit:.4f}
      wₐ (fitted) = {wa_fit:.4f}

    Comparison to Surveys:
      DESI 2024 BAO:      w_0 = -0.55 +/- 0.21,  wₐ = -1.75 +/- 0.65
      DESI 2024 Combined: w_0 = -0.83 +/- 0.06,  wₐ = -0.75 +/- 0.29
      Planck 2018:        w_0 = -1.03 +/- 0.03,  wₐ ~ 0 (unconstrained)
    """)
    ...
    Verdict: {'CONSISTENT' if abs(d_w0) < 2 and abs(d_wa) < 2 else 'TENSION'} with DESI at 2sigma
    """)
    ...
    # 1. w0-wa plane
    print("\n1. w_0-wₐ plane with survey constraints...")
    ...
    1. VACUUM OSCILLATION: Dark energy oscillates at gamma_1 = 14.13 in log-time
       -> Amplitude: ~2.6% variation in Lambda
       -> Current phase: 162 deg ("exhale" - tension decreasing)

    2. EFFECTIVE CPL PARAMETERS: w_0 = {w0_fit:.3f}, wₐ = {wa_fit:.3f}
       -> This is {'WITHIN' if abs(d_w0) < 2 else 'OUTSIDE'} DESI 2sigma contour
       -> The "DESI anomaly" (w_0 > -1, wₐ < 0) is {'EXPLAINED' if wa_fit < -0.3 else 'PARTIALLY MATCHED'}
