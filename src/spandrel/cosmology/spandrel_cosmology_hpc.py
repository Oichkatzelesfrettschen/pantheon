...
class VectorizedCosmology:
    ...
    def E_squared(self, z: np.ndarray) -> np.ndarray:
        """
        Vectorized E^2(z) = H^2(z)/H_0^2 for flat wCDM cosmology.
        ...
        if self.params.w0 == -1.0 and self.params.wa == 0.0:
            ...
            # rho_DE ∝ (1+z)^(3(1+w0+wa)) * exp(-3*wa*z/(1+z))
    ...
    def comoving_distance_vectorized(self, z: np.ndarray, n_steps: int = 1000) -> np.ndarray:
        ...
        D_C(z) = (c/H_0) * integral_0ᶻ dz'/E(z')
    ...
    def distance_modulus_vectorized(self, z: np.ndarray, n_steps: int = 1000) -> np.ndarray:
        """
        Vectorized distance modulus mu = 5*log_1_0(D_L/10pc).
        """
    ...
    def distance_modulus_spandrel_vectorized(self, z: np.ndarray, n_steps: int = 1000) -> np.ndarray:
        ...
        mu_spandrel = mu_LambdaCDM + Δmu_stiffness
        """
...
class ParallelMCMC:
    ...
    def log_likelihood(self, params: np.ndarray, use_spandrel: bool = True) -> float:
        ...
        params: [H0, Omega_m, epsilon] or [H0, Omega_m] for LambdaCDM
        """
        if len(params) == 3:
...
class NestedSampler:
    ...
    This allows proper model comparison between LambdaCDM and Spandrel cosmology
...
class SpandrelAnalysisPipeline:
    ...
    def _fit_mle_model(self, use_spandrel: bool) -> FitResult:
        ...
        print(f"  H_0 = {H0_fit:.3f} km/s/Mpc")
        print(f"  Omegaₘ = {Om_fit:.5f}")
        if use_spandrel:
            print(f"  epsilon = {eps_fit:.6f}")
        print(f"  chi^2 = {chi2_val:.2f} (dof = {dof})")
        print(f"  chi^2/dof = {reduced_chi2:.5f}")
    ...
    def compute_evidence(self, model: str = 'both',
        ...
        if 'lcdm' in results and 'spandrel' in results:
            ...
            if log_bayes > 1:
                print("  -> Strong evidence for Spandrel over LambdaCDM")
            elif log_bayes > 0:
                print("  -> Weak evidence for Spandrel over LambdaCDM")
            elif log_bayes > -1:
                print("  -> Weak evidence for LambdaCDM over Spandrel")
            else:
                print("  -> Strong evidence for LambdaCDM over Spandrel")
    ...
    def likelihood_ratio_test(self) -> Dict[str, float]:
        ...
        print("\n" + "="*70)
        print("LIKELIHOOD RATIO TEST: LambdaCDM vs Spandrel")
        print("="*70)
        ...
        if delta_chi2 > 3.84:  # 95% confidence
            ...
            print("\n  LambdaCDM is statistically sufficient")
    ...
    def summary_report(self):
        ...
        if 'mle' in self.results:
            ...
            for name, result in self.results['mle'].items():
                ...
                if result.params.epsilon != 0:
                    print(f"  epsilon = {result.params.epsilon:.6f}")
                print(f"  chi^2/dof = {result.reduced_chi2:.5f}")
        ...
        if 'mcmc' in self.results:
            ...
            for name, result in self.results['mcmc'].items():
                ...
                if result['diagnostics']['converged']:
                    print("  [OK] Chains converged (R̂ < 1.1)")
                else:
        ...
        if 'evidence' in self.results:
            ...
            if 'lcdm' in self.results['evidence'] and 'spandrel' in self.results['evidence']:
                ...
                print(f"\n  log(Bayes Factor Spandrel/LambdaCDM) = {log_bf:.4f}")
        ...
        if 'mcmc' in self.results and 'spandrel' in self.results['mcmc']:
            ...
            if abs(eps_mean) < 2 * eps_std:
                print(f"\n  epsilon = {eps_mean:.6f} +/- {eps_std:.6f}")
                print("  Result: epsilon is consistent with zero within 2sigma")
                print("  Interpretation: The Universe is purely Associative.")
                print("  Standard LambdaCDM appears sufficient.")
            elif eps_mean > 0:
                print(f"\n  epsilon = {eps_mean:.6f} +/- {eps_std:.6f}")
                print("  Result: epsilon > 0 detected!")
