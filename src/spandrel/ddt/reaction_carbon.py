...
# Effective Q-value (weighted average of channels, including alpha-capture)
...
def c12_c12_rate(T: np.ndarray) -> np.ndarray:
    ...
    Returns rate coefficient lambda (cm^3/s) such that:
        reaction_rate = (n_C12)^2 * lambda / 2
    ...
    # log10(N_A <sigmav>) = a0 + a1/T9 + a2/T9^(1/3) + a3*T9^(1/3) + a4*T9 + a5*T9^(5/3) + a6*ln(T9)

    # Simplified form using the resonance formula:
    # <sigmav> = S(E_0) / (E_0^2 T^(2/3)) * exp(-3 E_G / T^(1/3))
    # where E_G is the Gamow energy

    # Gamow peak energy for C12+C12
    # E_G = (Z1 Z2 e^2 pi / hbar)^2 * mu / 2  ~ 2.39 MeV at T9=1
    ...
    # log10(N_A <sigmav>) ~ 25.5 + term1/ln(10) + term2/ln(10) for T9~1
    log10_rate = 25.0 + term1 / np.log(10) + term2 / np.log(10)

    # Convert from cm^3/mol/s to cm^3/s per particle pair
    # lambda = N_A <sigmav> / N_A^2 = <sigmav> / N_A
...
def reaction_rate_c12(rho: np.ndarray, T: np.ndarray, X_C12: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Reaction rate (reactions per cm^3 per second)
    # Rate = n1 * n2 * lambda for different species
    # Rate = n^2 * lambda / 2 for identical species (symmetry factor)
    rate_density = 0.5 * n_C12**2 * lambda_C12 * f_screen

    # Mass consumption rate of C12 (g/cm^3/s)
    ...
    # Energy generation rate (erg/cm^3/s)
...
def burn_substep(rho: np.ndarray, e_int: np.ndarray, X_C12: np.ndarray,
    ...
    if method == 'forward_euler':
        ...
        # Approximate Jacobian: df/dX ~ 2 * dX_dt / X (since rate ∝ X^2)
        ...
        # For Numba compatibility in nopython mode, raising exceptions is tricky if not simple.
        # But since we only pass 'backward_euler' in practice, this branch might be optimized out.
        # Ideally, we return failure or use a default.
        return e_int, X_C12
...
if __name__ == "__main__":
    ...
    # Test conditions
    rho_test = np.array([1e7, 2e7, 5e7])       # g/cm^3
    ...
    for rho in rho_test:
        ...
        for T in T_test:
            ...
            print(f"  T = {T:.1e} K: epsilon = {eps[0]:.2e} erg/g/s, tau_burn = {tau_burn:.2e} s")
    ...
    print("\n" + "=" * 60)
    print("Burning Integration Test (dt = 10⁻^4 s)")
