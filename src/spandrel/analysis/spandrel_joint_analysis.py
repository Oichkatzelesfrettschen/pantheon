...
Breaking the Omegaₘ-epsilon degeneracy using external cosmological priors.
...
The Problem:
  ...
  - High Omegaₘ + Low epsilon ~ Low Omegaₘ + High epsilon

The Solution:
  Apply Gaussian priors from BAO/CMB measurements to "pin" Omegaₘ,
  forcing the stiffness parameter epsilon to reveal itself.

Priors Used:
  - Planck 2018 (CMB): Omegaₘ = 0.315 +/- 0.007
  - DESI BAO 2024: Omegaₘ = 0.295 +/- 0.015 (optional, shows tension)
...
class SpandrelCosmologyEngine:
    ...
    def distance_modulus_lcdm(self, z: np.ndarray) -> np.ndarray:
        """Standard LambdaCDM distance modulus."""
...
class JointAnalysisFitter:
    ...
    The total chi-squared is:
        chi^2_total = chi^2_SN + Σ chi^2_prior
    ...
    def fit(self, use_spandrel: bool = True,
        ...
        """
        model_name = "Spandrel" if use_spandrel else "LambdaCDM"
...
class OscillatorySpandrelEngine(SpandrelCosmologyEngine):
    ...
    Δmu = epsilon * sin(omega * ln(1+z)) * exp(-gamma * z) * f(z)

    where:
    - omega: oscillation frequency in log-redshift
    - gamma: decay rate
...
def run_degeneracy_analysis():
    ...
    2. Planck prior on Omegaₘ - breaks degeneracy
    3. SH0ES prior on H_0 - alternative anchor
    4. Combined priors - tightest constraints
    """

    print("\n" + "="*70)
    print("SPANDREL JOINT ANALYSIS: Breaking the Omegaₘ-epsilon Degeneracy")
    print("="*70)
    ...
    print(f"\n  LambdaCDM Free:     H_0 = {results['lcdm_free']['H0']:.3f}, "
          f"Omegaₘ = {results['lcdm_free']['Omega_m']:.4f}")
    print(f"  Spandrel Free: H_0 = {results['spandrel_free']['H0']:.3f}, "
          f"Omegaₘ = {results['spandrel_free']['Omega_m']:.4f}, "
          f"epsilon = {results['spandrel_free']['epsilon']:.6f}")

    delta_Om = results['spandrel_free']['Omega_m'] - results['lcdm_free']['Omega_m']
    print(f"\n  [WARN]  Matter density shift: ΔOmegaₘ = {delta_Om:+.4f}")
    print(f"      This {abs(delta_Om)/results['lcdm_free']['Omega_m']*100:.1f}% shift absorbs the stiffness!")

    # =========================================================================
    # Analysis 2: Planck prior on Omegaₘ (The "Cosmic Vise")
    # =========================================================================
    print("\n" + "-"*70)
    print("ANALYSIS 2: Planck CMB Prior (Omegaₘ = 0.315 +/- 0.007)")
    print("-"*70)
    ...
    print(f"\n  LambdaCDM + Planck:     H_0 = {results['lcdm_planck']['H0']:.3f}, "
          f"Omegaₘ = {results['lcdm_planck']['Omega_m']:.4f}")
    print(f"  Spandrel + Planck: H_0 = {results['spandrel_planck']['H0']:.3f}, "
          f"Omegaₘ = {results['spandrel_planck']['Omega_m']:.4f}, "
          f"epsilon = {results['spandrel_planck']['epsilon']:.6f}")

    print(f"\n  chi^2_prior contribution: {results['spandrel_planck']['chi2_prior']:.2f}")

    # =========================================================================
    # Analysis 3: DESI BAO prior (newer, slightly different)
    # =========================================================================
    print("\n" + "-"*70)
    print("ANALYSIS 3: DESI BAO 2024 Prior (Omegaₘ = 0.295 +/- 0.015)")
    print("-"*70)
    ...
    print(f"\n  Spandrel + DESI: H_0 = {results['spandrel_desi']['H0']:.3f}, "
          f"Omegaₘ = {results['spandrel_desi']['Omega_m']:.4f}, "
          f"epsilon = {results['spandrel_desi']['epsilon']:.6f}")

    # =========================================================================
    # Analysis 4: Combined Priors (Maximum constraint)
    # =========================================================================
    print("\n" + "-"*70)
    print("ANALYSIS 4: Combined Priors (Planck Omegaₘ + SH0ES H_0)")
    print("-"*70)
    print("  Testing for internal consistency with local H_0...")
    ...
    print(f"\n  Spandrel Combined: H_0 = {results['spandrel_combined']['H0']:.3f}, "
          f"Omegaₘ = {results['spandrel_combined']['Omega_m']:.4f}, "
          f"epsilon = {results['spandrel_combined']['epsilon']:.6f}")
    print(f"  chi^2_prior = {results['spandrel_combined']['chi2_prior']:.2f} "
    ...
    print(f"""
    Analysis                      epsilon (stiffness)      Omegaₘ (matter)
    -------------------------------------------------------------
    Free fit (degeneracy)         {results['spandrel_free']['epsilon']:+.6f}       {results['spandrel_free']['Omega_m']:.4f}
    + Planck Omegaₘ prior             {results['spandrel_planck']['epsilon']:+.6f}       {results['spandrel_planck']['Omega_m']:.4f}
    ...
    MCMC with Planck prior:
      epsilon = {mcmc_result['stats']['epsilon']['mean']:.6f} +/- {mcmc_result['stats']['epsilon']['std']:.6f}
    ...
    if significance < 2:
        print(f"""
    Detection Significance: {significance:.2f}sigma (NOT SIGNIFICANT)

    Result: epsilon is STILL consistent with zero even after breaking the
            Omegaₘ-epsilon degeneracy with Planck priors.
    ...
    Next Steps:
      ...
      3. Look for stiffness in the early universe (z > 2)
        ...
        print(f"""
    Detection Significance: {significance:.2f}sigma (POTENTIALLY SIGNIFICANT!)

    Result: epsilon > 0 detected after degeneracy breaking!

    Interpretation: With Omegaₘ pinned to Planck values, the data
    ...
    Physical Meaning:
      ...
      - This could partially explain the Hubble tension
        ...
        print(f"""
    Detection Significance: {significance:.2f}sigma

    Result: epsilon < 0 detected (universe "softening")
    ...
    if delta_chi2 > 3.84:
        ...
        print("\n  LambdaCDM remains statistically sufficient")
