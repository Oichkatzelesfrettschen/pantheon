...
def experiment_critical_gradient(verbose: bool = True) -> Dict:
    ...
    for i, width in enumerate(gradient_widths):
        if verbose:
            print(f"\n  [{i+1}/{len(gradient_widths)}] Testing lambda = {width/1e5:.0f} km...")
        ...
        status = "DDT [OK]" if solver.detonation_detected else "No DDT"
        if verbose:
    ...
    if verbose:
        print(f"\n  RESULT: lambda_crit ~ {lambda_crit:.1f} km")
...
def experiment_fractal_sweep(verbose: bool = True) -> Dict:
    ...
    if verbose:
        print(f"\n  {'D':>6} {'M_Ni':>8} {'Δm_1₅':>8} {'M_B':>8} {'S_T/S_L':>10}")
        print("  " + "-" * 45)
...
def experiment_light_curves(verbose: bool = True) -> Dict:
    ...
    for M_Ni, label in zip(M_Ni_values, D_labels):
        ...
        if verbose:
            print(f"  {label}: M_Ni = {M_Ni/M_SUN:.2f} MSun -> "
                  f"M_B = {obs['M_B_peak']:.2f}, Δm_1₅ = {obs['delta_m15']:.2f}")
...
def experiment_population(verbose: bool = True) -> Dict:
    ...
    for D_std in [0.05, 0.10, 0.15, 0.20]:
        ...
        if verbose:
            print(f"  sigma_D = {D_std:.2f}: Intrinsic scatter = {scatter['scatter']:.3f} mag")
...
def create_unified_figure(exp1: Dict, exp2: Dict, exp3: Dict, exp4: Dict,
    ...
    ax1.axvline(exp1['lambda_crit_km'], color='yellow', linestyle=':',
               linewidth=2, label=f'lambda_crit = {exp1["lambda_crit_km"]:.0f} km')
    ...
    ax3.set_ylabel('Ni-56 Mass (MSun)')
    ...
    pop = exp4['populations'][1]['population']  # sigma_D = 0.10
    ax7.hist2d(pop['delta_m15'], pop['M_B'], bins=25, cmap='inferno')
    ax7.plot(dm15, M_B, 'lime', linewidth=2, alpha=0.7)
    ax7.set_xlabel('Δm_1₅(B)')
    ax7.set_ylabel('M_B')
    ax7.set_title('Exp 4: Population (sigma_D=0.10)')
    ax7.invert_yaxis()
    ...
    ax8.set_xlabel('sigma_D (Turbulence Spread)')
    ...
    |    │  TURBULENCE  │     │  DIMENSION D │     │   LENGTH lambda   │     │  PROBABILITY │           |
    |    +--------------+     +--------------+     +--------------+     +--------------+           |
    |           │                    │                    │                    │                   |
    |           │                    │                    │                    │                   |
    |           ▼                    ▼                    ▼                    ▼                   |
    ...
    |                                                                                               |
    |  KEY RESULTS:                                                                                |
    |  * Critical gradient: lambda_crit ~ 12-15 km                                                      |
    |  * Fractal dimension range: D in [2.1, 2.6]                                                   |
    |  * Ni-56 yield: 0.3 - 1.1 MSun                                                                |
    |  * Phillips slope emerges NATURALLY from D variation                                         |
    |  * Intrinsic scatter (sigma ~ 0.15 mag) from turbulence distribution                            |
    |                                                                                               |
...
def run_all_experiments():
    ...
    create_unified_figure(exp1, exp2, exp3, exp4,
                         save_path=Path(__file__).parents[3] / "results" / "figures" / "unified_synthesis.png")
