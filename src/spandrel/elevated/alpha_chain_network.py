...
Each step proceeds via alpha-capture or photodisintegration at high T.
...
def alpha_capture_rate(T: float, target: Isotope) -> float:
    """
    alpha-capture reaction rate coefficient lambda (cm^3/s).

    target + alpha -> product + gamma
    ...
    S_factors = {
        Isotope.C12:  1.7e17,   # C12(alpha,gamma)O16
        Isotope.O16:  1.2e20,   # O16(alpha,gamma)Ne20
        Isotope.Ne20: 4.1e26,   # Ne20(alpha,gamma)Mg24
        Isotope.Mg24: 4.0e26,
        Isotope.Si28: 2.7e27,
        Isotope.S32:  1.6e28,
        Isotope.Ar36: 2.8e29,
    ...
    # Rate: N_A <sigmav> = S * T9^(-2/3) * exp(-tau)
...
def q_value(reactant: Isotope, product: Isotope) -> float:
    """
    Q-value (energy release) for alpha-capture reaction in erg.

    Q = (B_product × A_product - B_reactant × A_reactant - B_alpha × 4) × A_product
    Simplified: Q ~ ΔB × 4 (energy released per alpha captured)
    """
...
class AlphaChainNetwork:
    """
    13-isotope alpha-chain nuclear reaction network.
...
if __name__ == "__main__":
    print("=" * 70)
    print("alpha-CHAIN NUCLEAR NETWORK TEST")
    print("=" * 70)
    ...
    for rho, T, desc in test_cases:
        print(f"\n{'-' * 50}")
        print(f"{desc}")
        print(f"rho = {rho:.1e} g/cm^3, T = {T:.1e} K")
        print(f"{'-' * 50}")
        ...
        print(f"Equivalent Ni-56 mass (1.4 MSun WD): {M_Ni/M_SUN:.2f} MSun")
