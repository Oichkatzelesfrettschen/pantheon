...
Implements proper Bayesian model selection using:
    ...
    2. Multiple competing models (LambdaCDM, wCDM, CPL, Riemann)
    3. Information criteria (AIC, BIC, DIC)
    4. Posterior predictive checks

This elevates the original falsification from chi^2 comparison to
...
class NestedSampler:
    ...
    Computes:
        Z = integral L(theta) pi(theta) dtheta  (Evidence)
    ...
    def log_likelihood(self, params: np.ndarray) -> float:
        ...
        for i, zi in enumerate(z):
            ...
            D_C[i] = C_LIGHT / H0 * np.trapezoid(1/E_int, z_int)
...
def run_full_model_comparison(data_path: str = None) -> Dict:
    ...
    if data_path:
        ...
        for i, zi in enumerate(z):
            ...
            D_C[i] = C_LIGHT / true_params[0] * np.trapezoid(1/E_int, z_int)
        D_L = D_C * (1 + z)
    ...
    print("\n" + "=" * 70)
    print("BAYES FACTORS (relative to LambdaCDM)")
    print("=" * 70)
