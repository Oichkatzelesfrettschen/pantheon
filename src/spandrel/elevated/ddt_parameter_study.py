...
class DDTParameterStudy:
    ...
    def scan_gradient_width(self, widths: np.ndarray,
        ...
        print(f"Fixed: rho = {rho:.1e} g/cm^3, T_hot = {T_hot:.1e} K")
    ...
    def _run_batch(self, params_list: List[ParameterPoint],
        ...
        if parallel:
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                ...
                for i, future in enumerate(as_completed(futures)):
                    try:
                        ...
                        print(f"  [{i+1}/{len(params_list)}] lambda={result.params.gradient_width/1e5:.0f}km, "
                              f"rho={result.params.rho_ambient:.1e}: {status}")
                    except Exception as e:
            ...
            for i, params in enumerate(params_list):
                ...
                results.append(result)
                status = "DDT" if result.detonation else "No DDT"
                print(f"  [{i+1}/{len(params_list)}] lambda={params.gradient_width/1e5:.0f}km: {status}")
...
def plot_gradient_scan(results: List[SimulationResult], save_path: str = None):
    ...
    ax.bar(['Detonation', 'No Detonation'], [np.sum(detonations), np.sum(~detonations)],
          color=['#3fb950', '#f85149'])
    ax.set_ylabel('Count')
    ax.set_title(f'DDT Success Rate (lambda_crit ~ {lambda_crit:.0f} km)')
...
def plot_2d_phase_diagram(gradient_widths: np.ndarray, densities: np.ndarray,
    ...
    ax.set_xlabel('Gradient Width (km)')
    ax.set_ylabel('Density (10⁷ g/cm^3)')
...
def run_parameter_study():
    ...
    print("\n" + "-" * 50)
    print("SCAN 1: Gradient Width (fixed rho = 2×10⁷ g/cm^3)")
    print("-" * 50)

    gradient_widths = np.linspace(5e5, 5e6, 12)  # 5-50 km
    results_1d = study.scan_gradient_width(gradient_widths, rho=2e7, parallel=True)

    plot_gradient_scan(results_1d, save_path=Path(__file__).parents[3] / "results" / "figures" / "gradient_scan.png")

    # 2. 2D Phase Diagram (Gradient Width vs Density)
    print("\n" + "-" * 50)
    print("SCAN 2: 2D Phase Diagram (Gradient Width vs Density)")
    print("-" * 50)

    widths_2d = np.linspace(5e5, 4e6, 8)       # 5-40 km
    densities_2d = np.linspace(1e7, 5e7, 5)    # 1-5 × 10^7 g/cm^3

    detonation_grid, Ni56_grid = study.scan_2d_grid(
        widths_2d, densities_2d, parallel=True
    )

    plot_2d_phase_diagram(
        widths_2d, densities_2d, detonation_grid, Ni56_grid,
        save_path=Path(__file__).parents[3] / "results" / "figures" / "ddt_phase_diagram.png"
    )
