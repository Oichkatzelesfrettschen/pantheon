...
Key observables:
    ...
    - Decline rate (Δm_1₅: magnitude decline in 15 days)
...
Q_CO56 = 3.73e6 * 1.602e-12   # MeV -> erg (gamma-rays + positrons)
...
def radioactive_heating(t: np.ndarray, M_Ni: float) -> np.ndarray:
    ...
    Q(t) = M_Ni × [epsilon_Ni × exp(-t/tau_Ni) + epsilon_Co × (exp(-t/tau_Co) - exp(-t/tau_Ni)) × tau_Ni/(tau_Ni - tau_Co)]
...
class ArnettModel:
    ...
    def luminosity(self, t: np.ndarray) -> np.ndarray:
        ...
        L(t) = exp(-(t/tau_m)^2) × integral_0ᵗ Q(t') × 2t'/tau_m^2 × exp((t'/tau_m)^2) dt'
        """
        L = np.zeros_like(t)
        ...
        for i, ti in enumerate(t):
            ...
            integral = np.trapezoid(integrand, t_prime)
    ...
    def magnitude(self, t: np.ndarray, band: str = 'bol') -> np.ndarray:
        ...
        For bolometric: M_bol = -2.5 log_1_0(L / L_sun) + 4.74
        """
        L = self.luminosity(t)
...
class LightCurveGenerator:
    ...
    def plot(self, save_path: str = None):
        ...
        ax.set_title(f'B-Band Light Curve (Δm_1₅ = {obs["delta_m15"]:.2f})')
        ax.legend()
        ax.grid(True, alpha=0.3)
        ax.invert_yaxis()
        ax.set_xlim(0, 100)
        ...
        +==========================================+
        |  Input Parameters:                       |
        |    M_Ni  = {obs['M_Ni']:.3f} MSun                     |
        |    M_ej  = {self.model.M_ej/M_SUN:.2f} MSun                      |
        |    v_exp = {self.model.v_exp:.1e} cm/s             |
        |    kappa     = {self.model.kappa:.2f} cm^2/g                   |
        +==========================================+
        |  Derived Observables:                    |
